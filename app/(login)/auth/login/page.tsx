...
<p className="text-[10px] text-neutral-400 tracking-[0.3em] mt-3 uppercase font-medium opacity-80 italic">
  Hammad Vision <span className="mx-2 text-amber-600 font-normal">|</span> Your Reality
</p>
        </div>
        ...
        <form className="flex flex-col w-full flex-grow" onSubmit={handleSubmit(onSubmit)}>
          ...
          <button
            type="submit"
            className="w-full h-16 bg-amber-600 text-white rounded-2xl mt-[2rem] mb-4 text-xl font-bold uppercase tracking-widest hover:bg-amber-700 transition-colors shadow-lg active:scale-[0.98]"
          >
            Initialize
          </button>
        <div className="text-center mt-2">
            <p className="text-xs text-neutral-500">Don't Have An Account? <Link href="/auth/signup" className="text-amber-600 font-bold">Sign In</Link></p>
          </div>
        </form>
      </div>
    </div>
